...
from backend.server.types import FileConfig, GeneratorResult
...
class Reader(RagitComponent):
    ...
    async def load(self, config: dict, fileConfig: FileConfig) -> list[Document]:
        """Convert fileConfig into Ragit Documents
        @parameter: fileConfig: FileConfig - FileConfiguration sent by the frontend
        @returns list[Document] - Ragit documents
        """
...
class Embedding(RagitComponent):
    ...
    async def vectorize(self, config: dict, content: list[str]) -> list[float]:
        """Embed ragit documents and its chunks to Weaviate
...
class Chunker(RagitComponent):
    ...
    async def chunk(
        ...
        """Split Ragit documents into chunks.
        @parameter: config : dict - Chunker Configuration
        @parameter: documents : list[Document] - List of Ragit documents to chunk
...
class Generator(RagitComponent):
    ...
    async def generate_stream(
        ...
        @returns Iterator[dict] - Token response generated by the Generator in this format {message:TOKEN, finish_reason:stop or empty}.
        """
