...
if production_key:
    msg.info(f"Ragit runs in {production_key} mode")
...
async def get_all_documents(payload: SearchQueryPayload):
    try:
        ...
        documents, total_count = await manager.weaviate_manager.get_documents(
            ...
            payload.pageSize,
            payload.page,
            properties=["title", "extension", "fileSize", "source", "meta"],
        )
...
async def reset_verba(payload: ResetPayload):
    ...
    try:
        ...
        msg.info(f"Resetting RAGit in ({payload.resetMode}) mode")

        return JSONResponse(status_code=200, content={})

    except Exception as e:
        msg.warn(f"Failed to reset Ragit {str(e)}")
